    ...
    class ExportFactures
    {
                ...
                using (OdbcConnection connection = Connexion.CreateOdbcConnextion())
                {
                    ...
                    OdbcCommand command = new OdbcCommand(QueryHelper.getCustomer(false, do_tiers), connection);
                    {
                        using (IDataReader reader = command.ExecuteReader())
                        {
                            ...
                            {
                                recapLinesList_new.Add(new CustomMailRecapLines(docRefMail, "Aucun Client trouvé", "", "", logFileName_export));
                                return null;
                            }
                        }
                    }
        ...
        private void UpdateDocumentVente(string do_piece, List<CustomMailRecapLines> recapLinesList_new)
        {
            ...
            catch (Exception e)
            {
                //Exceptions pouvant survenir durant l'exécution de la requête SQL
                Console.WriteLine("" + e.Message.Replace("[CBase]", "").Replace("[Simba]", " ").Replace("[Simba ODBC Driver]", "").Replace("[Microsoft]", " ").Replace("[Gestionnaire de pilotes ODBC]", "").Replace("[SimbaEngine ODBC Driver]", " ").Replace("[DRM File Library]", ""));
                recapLinesList_new.Add(new CustomMailRecapLines(do_piece, e.Message, e.StackTrace, "", logFileName_export));
            }
        }
        ...
        private Societe getInfoSociete(List<CustomMailRecapLines> recapLinesList_new)
        {
            try
            {
                using (OdbcConnection connection = Connexion.CreateOdbcConnextion())
                {
                    ...
                    recapLinesList_new.Add(new CustomMailRecapLines(docRefMail, "Aucun Société trouvé.", "", "", logFileName_export));
                    return null;
                }
        ...
        private string getGNLClientLivraison(string intitule, List<CustomMailRecapLines> recapLinesList_new)
        {
            try
            {
                using (OdbcConnection connection = Connexion.CreateOdbcConnextion())
                {
                    ...
                    recapLinesList_new.Add(new CustomMailRecapLines(docRefMail, "Client n'est pas trouvé!", "", "", logFileName_export));
                    return null;
                ...
                using (OdbcConnection connection = Connexion.CreateOdbcConnextion())
                {
                    ...
                    OdbcCommand command = new OdbcCommand(QueryHelper.getModeReglement(false, do_piece), connection);
                    {
                        using (IDataReader reader = command.ExecuteReader())
                        {
                            ...
                            {
                                recapLinesList_new.Add(new CustomMailRecapLines(docRefMail, "Aucun mode de réglement trouvé!", "", "", logFileName_export));
                                return null;
                            }
                        }
                    }
