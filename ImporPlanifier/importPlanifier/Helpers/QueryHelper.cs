...
namespace importPlanifier.Helpers
{
    ...
    public static class QueryHelper
    {
        ...
        public static string getArticleStock(bool sqlConnexion, string reference)
        {
            if (sqlConnexion)
            {
                return "SELECT AS_QteSto, AS_QteRes, AS_MontSto FROM " + getPrefix() + "F_ARTSTOCK WHERE AR_Ref = '" + reference + "' and DE_No = 1";
            }
            else
            {
                return "SELECT * FROM F_ARTSTOCK WHERE AR_Ref = '" + reference + "'";
            }
        }
